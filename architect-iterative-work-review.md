...
- **审查点**：`task: Wiki: allow` 在 frontmatter 放行，但 `Agent Delegation`(102–107) 未列 Wiki。已决策保留（靠模型自行发现 task 列表），不补 routing。
...
## Research, Design, And Delivery（180–186）
...
- **审查点**：第 1 条与 `Agent Delegation`(106 行 CodeReview 路由) 一致；无冲突。
